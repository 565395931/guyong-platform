...
class WhatsappSessionWebJSCore extends session_abc_1.WhatsappSession {
    ...
    getClientOptions() {
        ...
        const webVersion = '2.3000.1043180520-alpha';
        const cacheType = 'local';
        this.logger.info(`Using cache type: '${cacheType}', web version: '${webVersion}'`);
        if (cacheType === 'local') {
    ...
    async fetchContactProfilePicture(id) {
        ...
        try {
            ...
            console.warn(`getProfilePicUrl failed for ${contactId}, trying ProfilePicThumb fallback`, err?.message || err);
        }
        try {
            return await this.whatsapp.pupPage.evaluate(async (contactId) => {
                const WidFactory = window.require('WAWebWidFactory');
                const Collections = window.require('WAWebCollections');
                const wid = WidFactory.createWid(contactId);
                const profilePic = await Collections.ProfilePicThumb.find(wid);
                return profilePic?.imgFull || profilePic?.img || null;
            }, contactId);
        }
        catch (err) {
            console.warn(`ProfilePicThumb fallback failed for ${contactId}`, err?.message || err);
            return null;
        }
    }
    ...
    toWAMessage(message) {
        ...
        const _serialized = message.id._serialized || message.id.id || ""; let key; try { key = (0, ids_1.parseMessageIdSerialized)(_serialized); } catch(e) { key = { id: _serialized, fromMe: message.fromMe || false, remoteJid: undefined, participant: undefined }; }
        return {
