...
exports.LoadUtils = () => {
            ...
            const msg = new (window.require('WAWebCollections').Msg.modelClass)(
                {
                    ...
                    cannotBeRanked: (() => { try { return window.require('WAWebStatusGatingUtils').canCheckStatusRankingPosterGating(); } catch(e) { return false; } })(),
                },
            );
    ...
    window.WWebJS.getChats = async () => {
        ...
        const chatPromises = chats.map((chat) =>
            window.WWebJS.getChatModel(chat).catch(function(e) {
                console.warn("getChatModel failed:", e.message);
                return null;
            }),
        );
    ...
    window.WWebJS.getChatModel = async (chat, { isChannel = false } = {}) => {
        ...
        if (model.msgs && model.msgs.length) {
            ...
            if (chat.lastReceivedKey) {
                ...
                const loadedMessages = chat.msgs
                    .getModelsArray()
                    .filter((msg) => !msg.isNotification)
                    .sort((a, b) => b.t - a.t);
                lastMessage = loadedMessages[0] || null;
                if (lastMessage) {
                    console.warn('getChatModel: missing lastReceivedKey, using loaded chat.msgs last message for', chat.id?._serialized);
                }
            }
            lastMessage &&
                (model.lastMessage =
